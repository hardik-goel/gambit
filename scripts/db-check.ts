...
async function main(): Promise<void> {
  ...
  if (!url) {
    ...
    docker(
      "run", "-d", "--rm",
      "--name", CONTAINER,
      "-e", "POSTGRES_PASSWORD=gambit",
      "-e", "POSTGRES_DB=gambit",
      "-p", `${PORT}:5432`,
      "postgres:16-alpine"
    );
    started = true;
  ...
  try {
    ...
    check(
      ...
      unprotected.length ? `missing on ${unprotected.join(", ")}` : `${tables.length} tables`
    );
    ...
    const readable = new Set(policies.filter((p) => p.cmd === "SELECT").map((p) => p.tablename));
    check(
      "game state is readable by nobody but the service role",
      !readable.has("games"),
      readable.has("games") ? "games has a select policy" : "no select policy on games"
    );
    check("moves are service-role only too", !readable.has("game_moves"));
    check("the event log is readable, and filtered by seat", readable.has("game_events"));

    /* ---- the atomic append ---- */
    const room = "11111111-1111-1111-1111-111111111111";
    await db.query(
      `insert into public.rooms (id, code, game_id, status, seed) values ($1, 'ABCDEF', 'chess', 'playing', 'seed-1')`,
      [room]
    );

    const first = await db.query<{ version: number; first_seq: string; last_seq: string }>(
      `select * from public.append_game_events($1, $2, $3::jsonb, $4::jsonb, $5, $6::jsonb, $7)`,
      [
        room,
        0,
    ...
    check("the first append lands at version 1", first.rows[0]?.version === 1, `version ${first.rows[0]?.version}`);

    const { rows: stateRows } = await db.query<{ version: number; state: { ply: number } }>(
      `select version, state from public.games where room_id = $1`,
      [room]
    );
    check("it saved the state alongside the version", stateRows[0]?.state.ply === 1);

    const { rows: eventRows } = await db.query<{ event: { type: string }; visible_to: number[] | null }>(
      `select event, visible_to from public.game_events where room_id = $1 order by seq`,
      [room]
    );
    ...
    try {
      await db.query(
        `select * from public.append_game_events($1, $2, $3::jsonb, $4::jsonb, $5, null, null)`,
        [room, 0, JSON.stringify({ ply: 99 }), JSON.stringify([{ type: "move" }]), 0]
      );
    } catch (e) {
    ...
    const { rows: afterConflict } = await db.query<{ state: { ply: number } }>(
      `select state from public.games where room_id = $1`,
      [room]
    );
    check("and the refused write changed nothing", afterConflict[0]?.state.ply === 1);
    ...
    const { rows: privateRows } = await db.query<{ visible_to: number[] | null }>(
      `select visible_to from public.game_events where room_id = $1 order by seq desc limit 1`,
      [room]
    );
    check(
    ...
    await db.query(
      `select * from public.append_game_events($1, $2, $3::jsonb, $4::jsonb, $5, $6::jsonb, $7)`,
      [room, 2, JSON.stringify({ ply: 3 }), JSON.stringify([{ type: "move" }]), 0, JSON.stringify({ kind: "move" }), "key-1"]
    );
    const { rows: moveRows } = await db.query<{ count: string }>(
      `select count(*)::text as count from public.game_moves where room_id = $1 and idempotency_key = 'key-1'`,
      [room]
    );
    check("a repeated idempotency key writes one move, not two", moveRows[0]?.count === "1");

    /* ---- the code constraint ---- */
    let rejectedCode = false;
    try {
      await db.query(
        `insert into public.rooms (code, game_id, status, seed) values ('lower!', 'chess', 'lobby', 's')`
      );
    } catch {
      rejectedCode = true;
    }
    check("room codes are constrained to six upper-case characters", rejectedCode);
  } finally {
